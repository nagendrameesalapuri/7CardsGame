...
import { registerSurvivalTeamHandlers } from "./handlers/survivalTeamHandler";
...
export function initSocketIO(io: Server) {
  ...
  io.on("connection", (socket) => {
    ...
    console.log(
      `[Socket] ${(socket as any).username} connected (${socket.id})`,
    );
    ...
    if (!isSpectator) {
      ...
      PlayerProgress.findOne({ userId: uid })
        .then((p) => {
          if (p)
            computeAndCacheBadge(
              uid,
              p.achievements.map((a) => a.id),
            );
        })
        .catch(() => {});
    ...
    registerSurvivalTeamHandlers(io, socket);

    // Notify client of all active games they can resume
    if (!isSpectator) {
      const actives = getAllActiveGamesByUserId(uid);
      if (actives.length > 0) {
        socket.emit("game:can_resume", {
          roomCodes: actives.map((a) => a.roomCode),
        });
      }
    }
