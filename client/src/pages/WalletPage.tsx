...
const UPI_ID   = 'paytmqr5p0dyv@ptys';
const UPI_NAME = '7Cards Game';
...
function AddMoneyModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  const upiLink = `upi://pay?pa=${UPI_ID}&pn=${encodeURIComponent(UPI_NAME)}&am=${finalAmount}&cu=INR`;
  ...
  const handleSubmitUTR = async () => {
    ...
    if (!utr.trim() || utr.trim().length < 6) return toast.error('Enter a valid UTR number');
    setLoading(true);
    try {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
      ...
      <motion.div
        ...
        style={{ background: 'rgba(12,14,18,0.98)', border: '1px solid rgba(0,255,136,0.2)' }}
      >
        {/* Step tabs */}
        ...
        <div className="p-5 space-y-4">
          {step === 'pay' ? (
            <>
              <p className="text-sm text-dark-muted text-center">Choose amount, then scan QR with any UPI app</p>
              ...
              <div className="grid grid-cols-4 gap-2">
                {PRESET_AMOUNTS.map(p => (
                  <button key={p}
                    ...
                    className={clsx('rounded-xl py-2.5 text-sm font-bold transition-all',
                      !useCustom && amount === p
              ...
              <div className="flex flex-col items-center gap-3 py-1">
                ...
                <p className="text-xs text-dark-muted">Scan with GPay · PhonePe · Paytm · BHIM</p>
              </div>
              ...
              >
                <div>
                  <p className="text-[10px] text-dark-muted uppercase tracking-wider">UPI ID</p>
                  <p className="text-sm font-mono text-white">{UPI_ID}</p>
                </div>
                <span className="text-xs font-semibold" style={{ color: copied ? '#00ff88' : '#8b949e' }}>
                  {copied ? '✓ Copied' : 'Copy'}
                </span>
              </div>

              {isDev && (
                <button onClick={handleDevAdd} disabled={loading || finalAmount < 1}
              ...
              <button
                ...
                style={{ background: 'linear-gradient(135deg, #00ff88, #00cc6a)', color: '#0d1117' }}
              >
            ...
            <>
              <div className="text-center space-y-1">
                <p className="text-sm font-bold text-white">Enter UTR / Transaction ID</p>
                <p className="text-xs text-dark-muted">Find it in your UPI app under payment history</p>
              </div>

              <div className="rounded-xl p-3 text-center text-sm font-bold text-neon-green"
                style={{ background: 'rgba(0,255,136,0.06)', border: '1px solid rgba(0,255,136,0.15)' }}>
                Amount: ₹{finalAmount}
              </div>

              <div>
                <label className="text-xs text-dark-muted block mb-1.5">UTR / Reference Number</label>
                <input
                  type="text"
              ...
              <div className="flex gap-3">
                ...
                <button onClick={handleSubmitUTR} disabled={loading || !utr.trim()}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
      ...
      >
        ...
        {method === 'upi' ? (
          <div>
            <label className="text-xs text-dark-muted block mb-1">Your UPI ID</label>
        ...
        <div className="rounded-lg px-3 py-2 text-xs text-dark-muted"
          style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      <div className="max-w-lg mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
          ...
          <div className="relative rounded-3xl p-6 overflow-hidden"
            style={{ background: 'linear-gradient(135deg, rgba(0,255,136,0.12) 0%, rgba(0,150,255,0.08) 100%)', border: '1px solid rgba(0,255,136,0.25)' }}>
          ...
          {pendingWithdrawals.length > 0 && (
            <div className="rounded-2xl p-4"
              ...
              <p className="text-xs font-semibold text-red-400 uppercase tracking-wider mb-2">⏳ Withdrawal Under Review</p>
              {pendingWithdrawals.map((w: any) => (
                <div key={w._id} className="flex justify-between items-center py-1.5">
                  <div>
                    <p className="text-sm text-white font-medium">₹{w.amount}</p>
                    <p className="text-xs text-dark-muted">{w.upiId ?? w.bankDetails?.accountName}</p>
                  </div>
              ...
              <p className="text-xs text-dark-muted mt-2">Admin will process within 24 hours.</p>
            </div>
          )}
