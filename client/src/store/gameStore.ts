...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  resumeGame: (code) => {
    set((s) => ({
      resumeRoomCodes: s.resumeRoomCodes.filter((c) => c !== code),
    }));
  ...
  respondToAttack: (action) => {
    const { selectedCardIds } = get();
    socketGame.attackRespond(
      action,
      action === "throw" ? selectedCardIds : undefined,
    );
    set({ selectedCardIds: [] });
  ...
  toggleCardSelection: (cardId) => {
    ...
    if (isSelected) {
      ...
      if (selectedCardIds.length === 0) {
        ...
        const firstSelected = game.myHand.find(
          (c) => c.id === selectedCardIds[0],
        );
  ...
  subscribeToEvents: () => {
    ...
    const handleConnect = () => {
      const { game, matchResult } = get();
      // Don't reconnect if the match already ended — the game is gone from the server
      if (game && !matchResult) {
    ...
    const handleVisibility = () => {
      if (document.visibilityState === 'visible') {
        const { game, matchResult } = get();
        // Don't reconnect after match end — the game is already gone from the server
        if (game && !matchResult) socketGame.reconnect(game.roomId);
      }
    };
    ...
    unsubs.push(
      on(
        ...
        ({
          ...
          if (codes.length > 0)
            set((s) => ({
              resumeRoomCodes: [...new Set([...s.resumeRoomCodes, ...codes])],
            }));
        },
      ),
    );
    ...
    unsubs.push(
      on("game:state", (incoming) => {
        ...
        const canShow =
          isMyTurn &&
          !incoming.hasDrawnThisTurn &&
          handTotal <= 5 &&
          !(
            incoming.attackChain?.targetPlayerIndex ===
            incoming.players.findIndex((p) => p.id === incoming.myPlayerId)
          );
        const underAttack = !!(
          incoming.attackChain &&
          incoming.players[incoming.attackChain.targetPlayerIndex]?.id ===
            incoming.myPlayerId
        );
        ...
        if (incoming.status === "playing") {
          incoming.players.forEach((player) => {
            if (player.id === incoming.myPlayerId || player.isEliminated)
              return;
        ...
        set((state) => {
          ...
          const localOnly =
            state.game?.chatMessages.filter((m) => !serverIds.has(m.id)) ?? [];
          ...
          return {
            ...
            ...(isNewGame
              ? { matchResult: null, lastAction: null, roundReadyUpdate: null }
              : {}),
          };
        });
      }),
      ...
      on("game:action", (action) => {
        ...
        switch (action.type) {
          ...
          case "discard":
            ...
            break;
          case "skip":
            soundService.play("power_jack");
            break;
          case "attack":
            soundService.play("power_seven");
            break;
          case "show":
            soundService.play("show_call");
            break;
    ...
    unsubs.push(
      on("game:round_ready_update", (update) => {
        set({ roundReadyUpdate: update });
      }),
    );

    unsubs.push(
      on("game:match_end", (result) => {
    ...
    unsubs.push(
      on("game:error", (msg) => {
        ...
        // Benign desync: attack chain resolved on server before our response arrived
        if (msg === "No active attack") {
          set((state) => ({
            underAttack: false,
            ...(state.game ? { game: { ...state.game, attackChain: null } } : {}),
          }));
          return;
        }
        notify.error(msg, {
          id: `game-err-${msg.replace(/\s+/g, "-").toLowerCase()}`,
          duration: 3000,
        });
      ...
      on("chat:received", (msg) => {
        ...
        set((state) => {
          ...
          return {
            game: {
              ...state.game,
              chatMessages: [...state.game.chatMessages, msg],
            },
          };
        });
      }),
    ...
    const handleForceEnded = ({ message }: { message: string }) => {
      ...
      set({
        ...
        room: null,
        matchResult: null,
        forceEndedMsg: message ?? "Game ended by admin",
      });
    };
    ...
    const handleAbandoned = ({ message }: { message: string }) => {
      notify.info(message ?? "Game abandoned — entry fees have been refunded", {
        duration: 7000,
      });
      set({
        game: null,
        room: null,
        matchResult: null,
        forceEndedMsg: message ?? "Game abandoned",
      });
    };
